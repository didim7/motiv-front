import { products5111 } from "@/data/products/fashion";
...
export default function Shop120() {
  const { toggleWishlist, isAddedtoWishlist } = useContextElement();
  const [selectedColView, setSelectedColView] = useState(4);

  const { addProductToCart, isAddedToCartProducts } = useContextElement();
  const [currentCategory, setCurrentCategory] = useState(menuCategoriespadel[0]);

  // eslint-disable-next-line no-unused-vars
  const [filtered, setFiltered] = useState(products5111);
  useEffect(() => {
    if (currentCategory == "All") {
      setFiltered(products5111);
    } else {
      setFiltered(
        products5111.filter((elm) => elm.filterCategory2 == currentCategory)
      );
    }
  }, [currentCategory]);
        ...
        >
          <div className="shop-banner position-relative">
            ...
            >
              <img
                loading="lazy"
                src="/assets/images/shop/cover3.jpeg"
                width="1759"
                height="420"
                alt="Pattern"
